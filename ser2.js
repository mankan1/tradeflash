...
async function buildOptionsWatchList(symbol, { expiries = [], moneyness = 0.25, limit = 150 } = {}) {
  ...
  let exps = expiries;
  if (!exps.length) exps = (await getExpirations(symbol)).slice(0, 1); // nearest expiry
  for (const exp of exps) {
    const { data } = await axios.get(`${TRADIER_BASE}/markets/options/chains`, {
      headers: H_JSON, params: { symbol, expiration: exp }
    });
    const all = data?.options?.option || [];
    for (const o of all) {
    ...
    es.onmessage = (e) => {
      try {
        ...
        if (msg?.type === "quote" && msg?.data) {
          ...
          // Emit synthetic option T&S when an OCC quote has last+size
...
app.get("/watch", async (req, res) => {
  try {
    ...
    // Build OCC list for all equities
    ...
    // Start streaming quotes for equities + OCC options
    startQuoteStream([...symbols, ...occSet]);

    // Backfill equity T&S for selected names
    if (backfillMins > 0) {
...
app.listen(Number(PORT), () => {
  ...
  console.log(`Try: curl "http://localhost:${PORT}/watch?symbols=SPY&eqForTS=SPY&backfill=15&day=2025-10-24&limit=200&moneyness=0.25"`);
});
