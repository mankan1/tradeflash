...
function inferSideServer(sym, price) {
  ...
  if (book) {
    ...
    const eps = mid ? Math.max(0.001 * mid, 0.01) : 0.01; // 0.1% or a cent
...
async function backfillEquityTS(symbol, day, minutes=5) {
  try {
    ...
    (data?.series?.data ?? []).forEach(tick => {
      ...
      broadcast({ type: "equity_ts", symbol, data: { ...tick, side, side_src } });
    });
  } catch (e) {
    ...
    es.onmessage = (e) => {
      try {
        ...
        if (msg?.type === "quote" && msg?.data) {
          ...
          // If this quote contains a trade-like update (last+size), assign side and broadcast
          if (q.last && q.size && q.size > 0) {
            ...
            // Quote without trade data — still broadcast if your UI uses NBBO (optional)
