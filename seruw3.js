  ...
  try {
    ...
    es.onmessage = (e) => {
      try {
        ...
        const q = msg.data; // { symbol, last, bid, ask, size, volume, trade_time, ... }
        // keep the book fresh for *both* equities and options
        updateMidFromQuote(q);

        // We only act when a trade prints
        if (q.last && q.size && q.size > 0) {
          ...
          if (isOCC) {
            ...
            // ========= EQUITY time & sales (NEW: 'at' tagging from live book) =========
            const book = midBySym.get(rootSym) || {};
            const eps = epsFor(book);
            let at = "between";
            if (book.ask && price >= book.ask - eps) at = "ask";
            else if (book.bid && price <= book.bid + eps) at = "bid";
            ...
            broadcast({
              ...
              symbol: rootSym,
              data: {
                time: q.trade_time,        // if provided by provider
                price,
                size: q.size,
                side,
                side_src,
...
app.get("/scan", async (req, res) => {
  try {
    ...
    const roots = symbols.length ? symbols : POPULAR;
