...
// ---------- heuristic open/close classifier ----------
const TH_CLOSE_BY_DVOL = 0.8; // ≥80% of OI already traded -> lean close
...
function classifyOpenClose({ qty, oi, priorVol, side, at }) {
  ...
  // Strong Open: traded size exceeds outstanding OI plus what already traded today.
  if (qty > (oi0 + prior)) {
    if (aggr === "buy") return { action: "BTO", action_conf: "high", rationale: "qty > OI + dayVol" };
    if (aggr === "sell") return { action: "STO", action_conf: "high", rationale: "qty > OI + dayVol" };
    return { action: "OPEN?", action_conf: "medium", rationale: "qty > OI + dayVol but aggressor unclear" };
  }

  // Closing lean: most of OI has already traded today.
  const dVolShare = prior / Math.max(1, oi0);
  if (dVolShare >= TH_CLOSE_BY_DVOL) {
    if (aggr === "buy")  return { action: "BTC", action_conf: "medium", rationale: "dayVol large vs OI; buyer-aggress" };
    if (aggr === "sell") return { action: "STC", action_conf: "medium", rationale: "dayVol large vs OI; seller-aggress" };
    return { action: "CLOSE?", action_conf: "low", rationale: "dayVol large vs OI; aggressor unclear" };
  }

  return { action: "—", action_conf: "low", rationale: "no strong open/close signal" };
}
...
async function buildOptionsWatchList(symbol, { expiries=[], moneyness=0.25, limit=150 }={}) {
  ...
  for (const exp of exps) {
    ...
    for (const o of arr) {
      oiByOcc.set(o.symbol, Number(o.open_interest || 0)); // cache OI
      if (Number.isFinite(Number(o.volume))) volByOcc.set(o.symbol, Number(o.volume)); // seed day vol
      ...
      try {
        ...
        if (msg?.type === "quote" && msg?.data) {
          ...
          if (q.last && q.size && q.size > 0) {
            ...
            // OCC option enriched T&S
            const isOCC = /^[A-Z]{1,6}\d{6}[CP]\d{8}$/i.test(q.symbol);
            if (isOCC) {
              // side/at using the OPTION book
              ...
              // prior volume (before this print)
